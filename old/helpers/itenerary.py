...
def get_itenerary(country, region_string, n_days, config):
    ...
    message_first_part = "Create a travel itenarary in json for {0} days in {1}, add these specific regions or cities: {2}.".format(
        n_days, country, region_string
    )
...
def get_city_description_chatgpt(country, region_string, config):
    openai.api_key = config["api_key"]
    message_first_part = (
        "Create a create a 100 word description of {0}, {1} for a tourist".format(
