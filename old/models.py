...
class Itenerary(db.Model):
    ...
    unique_search_history_id = db.Column(
        db.Integer,
        db.ForeignKey("unique_search_history.id", ondelete="CASCADE"),
        nullable=False,
    )
...
class CityDescriptors(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    city_id = db.Column(
        db.Integer, db.ForeignKey("world_cities.id", ondelete="CASCADE"), nullable=False
    )
...
class UniqueSearchHistory(db.Model):
    ...
    __table_args__ = (UniqueConstraint("country", "specific_places", "num_days"),)
...
class UserSavedItinerary(db.Model):
    ...
    __table_args__ = (UniqueConstraint("user_id", "ush_id", "from_date", "to_date"),)
